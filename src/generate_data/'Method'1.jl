function generate_data_set(
    ...
    #plso("agumentated_image_array")
    for index = 1:array_size
        original_image_size_tuple = size(input_original_image)
        #plso("original_image_size")
        #plso(v__(input_original_image))
        #plso(v__(output_original_image))
        #plso(original_image_size_tuple)
        #plso(size_tuple)
        #plso(agumentated_image_angle())
        #plso(agumentated_image_scale())
        #plso(Image_Noise())
    ...
    for index = 1:array_size
        #plso("convert_input")
        input_array[:, :, :, index] = convert_input(v__Dynamic_Array{Float32, 3}, agumentated_input_image_array[index], size_tuple)
        output_array[:, :, :, index] = convert_output(v__Dynamic_Array{Float32, 3}, agumentated_output_image_array[index], size_tuple)
    end
