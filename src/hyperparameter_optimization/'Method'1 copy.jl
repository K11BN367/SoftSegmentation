    ...
    f = function (Tuple...)
        ...
        f1 = function ()
            ...
            logger = function (Error, Batch_array_size, Model, Parameters, State, input_array, target_output_array)
                ...
                plso("logger Index_Update")
                if Index_Update >= 100
                    plso("logger Index_Update >= 100")
                    input_image = convert_input(v__Dynamic_Array{Gray{Float32}, 2}, input_array[:, :, :, 1] |> CPU_Device);
                    plso("input_image")
                    current_output_array, State = Model(input_array[:, :, :, 1:1], Parameters, State)
                    plso("current_output_array")
                    ...
                    plso("target_output_array")
        
                    output_1_image = convert_output(v__Dynamic_Array{Gray{Float32}, 2}, current_output_array[:, :, 1]);
                    output_2_image = convert_output(v__Dynamic_Array{Gray{Float32}, 2}, current_output_array[:, :, 2]);
                    output_3_image = convert_output(v__Dynamic_Array{Gray{Float32}, 2}, current_output_array[:, :, 3]);
